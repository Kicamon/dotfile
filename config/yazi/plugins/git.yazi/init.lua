...
local save = ya.sync(
	function(st, git_branch, git_file_status, git_is_dirty, git_status_str, is_ignore_dir, is_untracked_dir)
...
local M = {
	...
	entry = function(_, _)
		...
		local git_branch
		local result, _ = Command("git"):args({ "symbolic-ref", "HEAD" }):stdout(Command.PIPED):output()
