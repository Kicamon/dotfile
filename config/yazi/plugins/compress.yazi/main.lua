  ...
  for pattern, cmd_list in pairs(archive_commands) do
    if output_name:match(pattern) then
      for _, cmd in ipairs(cmd_list) do
        ...
        local cmd_name = type(cmd.command) == 'table' and find_command_name(cmd.command)
          or cmd.command
...
return {
  entry = function(_, job)
    ...
    if not temp_dir_status then
      ...
      notify(
        string.format('Failed to create temp directory, error code: %s', tostring(temp_dir_err)),
        'error'
      )
      return
    end
