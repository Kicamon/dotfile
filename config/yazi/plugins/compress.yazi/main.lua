...
return {
  entry = function(_, job)
    ...
    output_name = output_name == '' and string.format('%s.%s', default_name, default_extension)
      or output_name
    ...
    for pattern, cmd_list in pairs(archive_commands) do
      if output_name:match(pattern) then
        ...
        for _, cmd in ipairs(cmd_list) do
          ...
          local find_command = type(cmd.command) == 'table' and find_command_name(cmd.command)
            or cmd.command
    ...
    if not archive_cmd then
      notify_error(
        'Could not find a suitable archive program for the selected file extension',
        'error'
      )
      return
    end
    ...
    if archive_level_arg ~= '' and is_level then
      ...
      then
        cmd_level = type(archive_level_arg) == 'table'
            and archive_level_arg[#archive_level_arg] .. output_level
    ...
    if not temp_dir_status then
      ...
      notify_error(
        string.format('Failed to create temp directory, error code: %s', temp_dir_err),
        'error'
      )
      return
    end
    ...
    for filepath, filenames in pairs(path_fnames) do
      ...
      if not archive_status or not archive_status.success then
        ...
        if not cleanup_status then
          notify_error(
            string.format(
              'Failed to clean up temporary directory %s, error: %s',
              temp_dir,
              cleanup_err
            ),
            'error'
